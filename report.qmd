...
proportions_data = final_pivot|>
    group_by(State)|>
    mutate(proportion = total_renewable_energy/(Coal + `Natural Gas†` + `Petroleum (BTU)`))|>
...
necessary_columns = joined_dataset|>
    select(c(1, 2, 5))
```
...
```{r}
ggplot(necessary_columns, aes(x = proportion, y = as.numeric(`2021_Average_Price`))) + geom_point() + geom_smooth(method = lm) + labs(title = "", x = "Proportion that is renewable energy", y = "Average Price of energy") 
```